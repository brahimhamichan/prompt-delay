...
const isPromptLaterDemoTarget = (targetUrl: string): boolean => {
  try {
    const url = new URL(targetUrl);
    return url.hostname === "prompt-later.pages.dev" || url.hostname === "prompt-later.pages.dev";
  } catch {
    return false;
  }
};
...
const recordPromptLaterDotDevScenario = async (page: Page): Promise<void> => {
...
const run = async () => {
  ...
  const useScriptedScenario = !manual && isPromptLaterDemoTarget(targetUrl);

  console.log(`Recording rrweb events from: ${targetUrl}`);
  console.log(`Output: ${OUTPUT_PATH}`);
  console.log(`Mode: ${useScriptedScenario ? "scripted prompt-later.pages.dev" : "manual"}\n`);
  ...
  try {
    if (useScriptedScenario) {
      ...
      await recordPromptLaterDotDevScenario(page);
    } else {
